...
class RGSamplerReal(nn.Module):
    ...
    def forward(self, x_tf, return_traj=False):
        ...
        Integrate from s=1(t_f) to s=0(t_s).
        """
        device = x_tf.device
...
class RGSamplerTotal(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, batch_size, return_intermediate=False):
        """
        return:
            x0: [B,1,L]
        """
        ...
        # Sample the prior noise at t=1.
        Lnow = self.L_list[-1]
        x_tf = self.rg_diffusion_real.sample_prior(batch_size, Lnow)
        assert x_tf.shape == (batch_size, 1, Lnow)
        ...
        for i in reversed(range(len(self.L_list))):
            ...
            if i > 0:
                ...
                t_now = sampler.t_s
                x_ts = self.rg_diffusion_real.chain_size_lifter(x_ts, t_now, L_now, L_large)
                x_tf = x_ts
                intermediate.append(x_tf)
...
class RGSamplerChecker(nn.Module):
    ...
    def __init__(self, rg_diffusion_real, sampler, L):
        super().__init__()
    ...
    def forward(self, phi_0, return_noisy=False):
        ...
        x_tf, _ = self.rg_diffusion_real(phi_0, t, resizeL=self.L)
