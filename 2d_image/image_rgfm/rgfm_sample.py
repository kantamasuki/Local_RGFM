"""Generate images with the complete multi-resolution local RGFM chain."""
...
def assert_chain(configs):
    # configs should be ordered large -> small: [L64, L32, L16]
...
def sample_total(args):
    ...
    ckpts = [args.ckpt64, args.ckpt32, args.ckpt16]

    samplers = [
        build_sampler(configs[0], ckpts[0], device, args.ckpt_key),
        build_sampler(configs[1], ckpts[1], device, args.ckpt_key),
        build_sampler(configs[2], ckpts[2], device, args.ckpt_key),
    ]
